//отвечает за требование ТЗ «Spring Data JPA» и «без N+1». Здесь всё общение с PostgreSQL. Для загрузки выдач используется JOIN FETCH — это значит что книга, экземпляр и читатель загружаются одним SQL-запросом, а не отдельным запросом для каждой записи. Также здесь нативные SQL-запросы для сложной фильтрации.
...
interface LoanRepository : JpaRepository<Loan, UUID> {
    ...
    @Query("""
        ...
        WHERE l.status IN ('ACTIVE', 'OVERDUE') AND l.dueDate < :today
    """)
