//регистрация читателей и вход в систему: проверяет пароль через BCrypt, при успехе генерирует JWT-токен.
...
class AuthService(
    ...
    fun refresh(request: RefreshTokenRequest): AuthResponse {
        ...
        val account = userAccountRepository.findByLogin(login)
            ?: throw BusinessRuleException("User not found")
        return buildAuthResponse(account)
    }
    private fun buildAuthResponse(account: UserAccount): AuthResponse {
        val readerId = if (account.role == UserRole.READER) {
            readerRepository.findByUserAccount(account)?.id?.toString()
        } else null
        return AuthResponse(
            accessToken  = jwtTokenProvider.generateAccessToken(account.login, account.role.name),
            refreshToken = jwtTokenProvider.generateRefreshToken(account.login, account.role.name),
            login = account.login,
