...
| Требование | Статус |
|------------|--------|
| REST API с документацией Swagger | Готово |
| JWT-аутентификация + роли LIBRARIAN/READER | Готово |
| Каталог книг с фильтрацией по жанру, автору, названию | Готово |
| Выдача и возврат книг с проверкой бизнес-правил | Готово |
| Автоматическая обработка просрочек при старте и в 00:00 | Готово |
| Штрафы — начисление, оплата, списание | Готово |
| Redis-кэш с автоматическим сбросом при операциях | Готово |
| Асинхронные уведомления через @Async | Готово |
| Flyway-миграции, схема поднимается автоматически | Готово |
| Unit-тесты (MockK) + интеграционные (Testcontainers) | Готово |
| Docker Compose — запуск одной командой | Готово |
| GitHub Actions CI/CD — все 4 джоба зелёные | Готово |
| Spring Actuator /health /metrics | Готово |
| Разграничение интерфейса по ролям на фронтенде | Готово |
