//страница выдач. Оформление выдачи, возврат книги, фильтрация по статусу (активные, просроченные).
<template>
  <div class="page">
    <div class="page-header">
      <h2>Выдачи</h2>
      <button v-if="authStore.isLibrarian" class="btn-primary" @click="openForm">+ Выдать книгу</button>
    </div>
    ...
    <div v-if="authStore.isLibrarian" class="filters">
    ...
    <div v-else class="loans-list">
      <div v-for="loan in loans" :key="loan.id" class="loan-card">
        ...
        <div class="loan-actions">
          <span class="status-badge" :class="loan.status?.toLowerCase()">{{ statusText(loan.status) }}</span>
          <button v-if="authStore.isLibrarian && (loan.status === 'ACTIVE' || loan.status === 'OVERDUE')" class="btn-return" @click="returnLoan(loan.id)">Вернуть</button>
        </div>
      </div>
    </div>
  </div>
...
async function loadLoans() {
  ...
  try {
    ...
    if (authStore.isLibrarian) {
      ...
      res = await api.get(`/readers/${authStore.readerId}/loans`)
    }
    loans.value = parseList(res.data).map((l: any) => ({
      ...l,
      bookTitle: l.bookTitle ?? l.bookCopy?.bookTitle?.title ?? '',
      readerName: l.readerName ?? l.reader?.fullName ?? ''
    }))
...
function statusText(s: string) {
  ...
  if (s === 'OVERDUE') return 'Просрочена'
  if (s === 'RETURNED') return 'Возвращена'
  return s
}
onMounted(loadLoans)
</script>
