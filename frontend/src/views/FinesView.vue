//страница штрафов. Список штрафов, кнопки оплаты и списания.
<template>
  <div class="page">
    ...
    <div v-else class="fines-list">
      <div v-for="fine in fines" :key="fine.id" class="fine-card">
        ...
        <div class="fine-actions">
          ...
          <button v-if="authStore.isLibrarian && fine.status === 'PENDING'" class="btn-pay" @click="payFine(fine.id)">Оплачен</button>
          <button v-if="authStore.isLibrarian && fine.status === 'PENDING'" class="btn-waive" @click="waiveFine(fine.id)">Списать</button>
        </div>
      </div>
    </div>
  </div>
...
async function loadFines() {
  ...
  try {
    ...
    if (authStore.isLibrarian) {
      ...
      res = await api.get(`/readers/${authStore.readerId}/fines`)
    }
    fines.value = parseList(res.data).map((f: any) => ({
      ...f,
      readerName: f.readerName ?? f.loan?.reader?.fullName ?? ''
    }))
