...
<template>
  <div>
    ...
    <div class="filters">
      <input v-model="search.title" placeholder="Поиск по названию" @input="loadBooks" />
      <input v-model="search.author" placeholder="Поиск по автору" @input="loadBooks" />
      <select v-model="search.genre" @change="loadBooks">
    ...
    <div v-if="showForm" class="modal-overlay" @click.self="showForm = false">
      <div class="modal">
        <h3>Новая книга</h3>
        <div class="field"><label>Название</label><input v-model="form.title" placeholder="Введите название" /></div>
        <div class="field"><label>Автор</label><input v-model="form.author" placeholder="Введите автора" /></div>
        <div class="field"><label>ISBN</label><input v-model="form.isbn" placeholder="Введите ISBN" /></div>
        <div class="field">
          <label>Жанр</label>
        ...
        <div class="field"><label>Год издания</label><input v-model="form.publishYear" type="number" placeholder="2024" /></div>
        <div class="field"><label>Количество экземпляров</label><input v-model="form.copies" type="number" placeholder="1" /></div>
...
const form = ref({ title: '', author: '', isbn: '', genre: '', publishYear: 2024, copies: 1 })
const search = ref({ title: '', author: '', genre: '' })
async function loadBooks() {
  loading.value = true
  try {
    const params = new URLSearchParams()
    if (search.value.title) params.append('title', search.value.title)
    if (search.value.author) params.append('author', search.value.author)
    if (search.value.genre) params.append('genre', search.value.genre)
...
async function addBook() {
  try {
    ...
    form.value = { title: '', author: '', isbn: '', genre: '', publishYear: 2024, copies: 1 }
    await loadBooks()
  } catch (e) {
    console.error(e)
  }
...
.filters input, .filters select { padding: 8px 12px; border: 1px solid #ddd; border-radius: 8px; font-size: 14px; min-width: 180px; }
...
.year { color: #888; font-size: 12px; margin-bottom: 0.3rem; }
.genre { display: inline-block; background: #f0f4ff; color: #4f6ef7; padding: 2px 10px; border-radius: 20px; font-size: 12px; margin-bottom: 0.5rem; }
...
.field input, .field select { width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 8px; font-size: 14px; box-sizing: border-box; }
